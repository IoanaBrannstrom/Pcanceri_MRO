...
function apply_blastp {
	...
	IN2=$my_DB 
	IN3=$append
	blastp -db $IN2 -query $IN1 -num_threads 1  -max_target_seqs 100 -evalue $e -outfmt 6 -out Blastp.${IN1}-vs-${IN3}_${e}.tab
...
#### 3. extract fasta based on the hits IDs
 function extract_fasta {
     ...
     IN1=$MyFasta
	 IN3=$append
	 blastdbcmd -db $my_DB -dbtype prot -entry_batch $list -outfmt %f -out ${IN1}-vs-${IN3}.pep
 }
