...
OUTDIR=  	#oath of the output directory
Append=	 #append the desire new name
MIN=5  #minim coverage kept
TARGET= #maxim coverage kept
...
bbnorm.sh in=$IN1 in2=$IN2  out=$MAPDIR_temp/${Append}_pe1.norm${TARGET}.fastq.gz out2=$MAPDIR_temp/${Append}_pe2.norm${TARGET}.fastq.gz target=$TARGET min=$MIN k=27 threads=10 qout=auto -Xmx58g
...
rsync -av $MAPDIR_temp/${Append}_pe1.norm${TARGET}.fastq.gz $OUTDIR
rsync -av $MAPDIR_temp/${Append}_pe2.norm${TARGET}.fastq.gz $OUTDIR
